...
async function setupAnalyticsCollections() {
  ...
  const collections = [
    {
      ...
      description: 'Tracks scenario views, completions, ratings, and user interactions'
    },
    {
      name: 'analytics_framework_engagement',
      description: 'Monitors philosophical framework selection and switching patterns'
    },
    {
      name: 'analytics_session_tracking',
      description: 'Captures user navigation, interactions, and session durations'
    },
    {
  ...
  try {
    for (const collection of collections) {
      ...
      console.log(`✅ ${collection.name}: ${snapshot.size} document(s) accessible`);
    }
