...
class ModalFooterManager {
    ...
    setupOverflowDetection(footer) {
        const checkOverflow = () => {
            ...
            footer.classList.toggle('has-overflow', hasHorizontalOverflow || hasVerticalOverflow);
    ...
    setupResponsiveBehavior(footer) {
        ...
        const handleResponsiveLayout = () => {
            ...
            if (!isMobile) {
                ...
                const needsStacking = totalButtonWidth + gapWidth + padding > footerWidth;
    ...
    setupAccessibility(footer) {
        ...
        const primaryButton = footer.querySelector('.modal-button.primary') || 
    ...
    addScrollIndicators(footer) {
        ...
        footer.addEventListener('scroll', () => this.updateScrollIndicators(footer));
    }
    ...
    updateScrollIndicators(footer) {
        ...
        const isAtEnd = footer.scrollLeft >= footer.scrollWidth - footer.clientWidth;
            ...
            mutations.forEach(mutation => {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        ...
                        const footers = node.querySelectorAll && node.querySelectorAll('.modal-footer');
                        if (footers) {
                            footers.forEach(footer => this.manageFooter(footer));
                        }
                    }
