...
class ScrollLockManager {
  ...
  lock(reason = "unknown") {
    try {
      ...
      if (this.lockCount === 1) {
        ...
        if (this.debugMode) {
          console.log(
            `[ScrollLockManager] Scroll locked by "${reason}" (count: ${this.lockCount})`,
          );
        }
      } else if (this.debugMode) {
        console.log(
          `[ScrollLockManager] Additional lock by "${reason}" (count: ${this.lockCount})`,
        );
      }
    } catch (error) {
  ...
  unlock(reason = "unknown") {
    try {
      if (this.lockCount <= 0) {
        if (this.debugMode) {
          console.warn(
            `[ScrollLockManager] Attempted to unlock scroll with "${reason}" but no locks exist`,
          );
        }
        return;
      }
      ...
      if (this.lockCount === 0) {
        ...
        if (this.debugMode) {
          console.log(
            `[ScrollLockManager] Scroll unlocked by "${reason}" - fully restored`,
          );
        }
      } else if (this.debugMode) {
        console.log(
          `[ScrollLockManager] Lock removed by "${reason}" (count: ${this.lockCount}, remaining: ${Array.from(this.lockReasons).join(", ")})`,
        );
      }
    } catch (error) {
  ...
  forceUnlock() {
    if (this.lockCount > 0) {
      if (this.debugMode) {
        console.warn(
          `[ScrollLockManager] Force unlock - had ${this.lockCount} locks from: ${Array.from(this.lockReasons).join(", ")}`,
        );
      }
  ...
  static emergencyScrollRestoration() {
    try {
      ...
      console.error(
        "[ScrollLockManager] Emergency scroll restoration failed:",
        error,
      );
      return false;
    }
        ...
        if (bodyOverflow === "hidden" && scrollLockManager.lockCount === 0) {
          ...
          setTimeout(() => {
            if (scrollLockManager.lockCount === 0) {
              document.body.style.overflow = "";
              console.warn(
                "[ScrollLockManager] Detected unauthorized scroll lock, restored",
              );
            }
...
if (typeof window !== "undefined") {
  ...
  window.emergencyScrollRestore = () =>
    ScrollLockManager.emergencyScrollRestoration();
}
