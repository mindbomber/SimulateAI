...
const INPUT_CONFIG = {
    ...
    KEYBOARD_SETTINGS: {
        ...
        ACCESSIBILITY_KEYS: ['Tab', 'Enter', 'Space', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight']
    },
...
export class InputManager {
    ...
    constructor(engine, options = {}) {
        ...
        this.touch = {
            ...
            maxTouches: 0,
            gestures: {
                pinch: { active: false, scale: 1, startDistance: 0, center: { x: 0, y: 0 } },
    ...
    applyTheme(themeName) {
        ...
        if (element) {
            element.style.setProperty('--input-focus-color', this.themeColors.focusColor);
            element.style.setProperty('--input-hover-color', this.themeColors.hoverColor);
        }
    ...
    setupMouseEvents() {
        ...
        this.boundMouseHandlers = {
            ...
            mousemove: this.throttle(this.handleMouseMove.bind(this), this.config.THROTTLE_INTERVAL),
    ...
    setupTouchEvents() {
        ...
        this.boundTouchHandlers = {
            ...
            touchmove: this.throttle(this.handleTouchMove.bind(this), this.config.THROTTLE_INTERVAL),
    ...
    setupWheelEvents() {
        ...
        this.boundWheelHandler = this.throttle(this.handleWheel.bind(this), this.config.THROTTLE_INTERVAL);
    ...
    processGamepadState(gamepad) {
        ...
        gamepad.buttons.forEach((button, index) => {
            ...
            const isPressed = button.pressed || button.value > this.config.GAMEPAD_SETTINGS.BUTTON_THRESHOLD;
        ...
        if (this.eventHandlers.has(eventType)) {
            this.eventHandlers.get(eventType).forEach(callback => {
                try {
                    ...
                    this.errorHandler.handleError(error, INPUT_ERRORS.HANDLER_ERROR, { eventType });
                }
            });
        }
    }
...
class InputPerformanceMonitor {
    ...
    updateMetrics() {
        ...
        if (recentEvents.length > 0) {
            const responseTime = recentEvents.reduce((sum, t) => sum + (now - t), 0) / recentEvents.length;
    ...
    detectAccessibilityPreferences() {
        if (window.matchMedia) {
            ...
            const updateReducedMotion = (e) => {
                ...
                this.inputManager.emit('accessibilityChanged', { reducedMotion: this.reducedMotion });
            };
            ...
            const updateHighContrast = (e) => {
                ...
                this.inputManager.emit('accessibilityChanged', { highContrast: this.highContrast });
            };
