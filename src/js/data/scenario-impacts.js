...
export function calculateCumulativeScores(answers) {
    ...
    for (const answer of answers) {
        const impact = getAnswerImpact(answer.categoryId, answer.scenarioId, answer.answerId);
        if (impact) {
            for (const [axis, delta] of Object.entries(impact.impacts)) {
                scores[axis] = Math.max(MIN_SCORE, Math.min(MAX_SCORE, scores[axis] + delta));
            }
        }
    }
 ...
 */
export function previewAnswerImpact(currentScores, categoryId, scenarioId, answerId) {
    const impact = getAnswerImpact(categoryId, scenarioId, answerId);
    if (!impact) return currentScores;
    
    const newScores = { ...currentScores };
    for (const [axis, delta] of Object.entries(impact.impacts)) {
        newScores[axis] = Math.max(MIN_SCORE, Math.min(MAX_SCORE, currentScores[axis] + delta));
    }
