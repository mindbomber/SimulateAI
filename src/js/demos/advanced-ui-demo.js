...
export class AdvancedUIDemo {
  constructor(container, options = {}) {
    ...
    this.options = {
      ...
      description:
        "Complete demonstration of SimulateAI's enhanced Interactive Object System",
      debug: true,
      ...options,
    };
  ...
  async init() {
    ...
    logger.info(
      'AdvancedUIDemo: Ready - Showcasing all advanced UI components'
    );
  }
  ...
  createTooltipSystem() {
    ...
    const tooltipBtn1 = this.engine.createComponent('button', {
      ...
      onMouseEnter: () =>
        this.showTooltip(
          'This button demonstrates tooltip functionality',
          tooltipBtn1
        ),
    ...
    const tooltipBtn2 = this.engine.createComponent('button', {
      ...
      text: 'Info Button',
      onMouseEnter: () =>
        this.showTooltip('Additional information appears here', tooltipBtn2),
      onMouseLeave: () => this.hideTooltip(),
    });
    ...
    const helpBtn = this.engine.createComponent('button', {
      ...
      text: 'Help',
      onMouseEnter: () =>
        this.showTooltip('Get help and support information', helpBtn),
      onMouseLeave: () => this.hideTooltip(),
    });
  ...
  showInfoModal() {
    const modal = this.engine.createComponent('modal-dialog', {
      ...
      content: `
            ...
            `,
      buttons: [{ text: 'Got it!', action: 'close', variant: 'primary' }],
    });
    modal.open();
  }
  ...
  updateChartData() {
    ...
    const newBarData = Array.from(
      { length: 7 },
      () => Math.floor(Math.random() * 60) + 20
    );
    const newPieData = Array.from(
      { length: 5 },
      () => Math.floor(Math.random() * 25) + 10
    );

    // Update chart data (this would be implemented in the Chart component)
    logger.debug('Updating chart data:', {
      newLineData,
      newBarData,
      newPieData,
    });
  ...
  updateStatus(message) {
    ...
    if (statusLabel) {
      statusLabel.setText(
        `Demo Status: ${message} | Keyboard: Ctrl+H (Help), Ctrl+R (Reset), Esc (Close)`
      );
    }
