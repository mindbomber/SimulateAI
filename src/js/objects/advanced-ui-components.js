...
class ModalDialog extends BaseObject {
    ...
    constructor(options = {}) {
        ...
        this.buttons = Array.isArray(options.buttons) ? options.buttons : [{ text: 'OK', action: 'close' }];
    ...
    async animateIn() {
        ...
        switch (this.animation) {
            case 'slide':
                this.y -= 100;
                animations.push(
                    this.animate('y', this.y + 100, UI_CONSTANTS.ANIMATION_DURATION.NORMAL, 'easeOutCubic'),
            ...
            case 'scale':
                ...
                this.scaleY = 0.8;
                animations.push(
                    this.animate('scaleX', 1, UI_CONSTANTS.ANIMATION_DURATION.NORMAL, 'easeOutBack'),
                    this.animate('scaleY', 1, UI_CONSTANTS.ANIMATION_DURATION.NORMAL, 'easeOutBack'),
                    this.animate('alpha', 1, UI_CONSTANTS.ANIMATION_DURATION.NORMAL)
                );
                break;
            ...
            case 'bounce':
                ...
                this.scaleY = 0.3;
                animations.push(
                    this.animate('scaleX', 1, UI_CONSTANTS.ANIMATION_DURATION.SLOW, 'bounce'),
                    this.animate('scaleY', 1, UI_CONSTANTS.ANIMATION_DURATION.SLOW, 'bounce'),
    ...
    handleTabNavigation(event) {
        ...
        if (event.shiftKey) {
            ...
            this.focusedButtonIndex = Math.min(this.buttonElements.length - 1, this.focusedButtonIndex + 1);
        }
    ...
    calculateButtonLayout() {
        ...
        const totalButtonWidth = this.buttonElements.reduce((sum, btn) => sum + btn.width, 0);
        ...
        this.buttonElements.forEach(button => {
            button.x = currentX;
            button.y = this.height - this.footerHeight + (this.footerHeight - button.height) / 2;
    ...
    renderHeader(renderer) {
        ...
        renderer.fillText(this.title, this.padding, headerY + this.headerHeight / 2);
    ...
    renderCloseButton(renderer) {
        ...
        // Button background
        renderer.fillStyle = this.closeButtonHovered ? UI_CONSTANTS.COLORS.GRAY_200 : 'transparent';
    ...
    renderBody(renderer) {
        ...
        if (this.content) {
            ...
            const lines = UIUtils.wrapText(this.content, maxWidth, '14px Arial', renderer);
    ...
    renderButton(renderer, button) {
        ...
        if (button.isPressed) {
            ...
            bgColor = button.variant === 'secondary' ? UI_CONSTANTS.COLORS.GRAY_400 : '#1976D2';
        }
    ...
    renderFocusRing(renderer) {
        ...
        renderer.strokeRect(-FOCUS_RING_WIDTH, -FOCUS_RING_WIDTH, 
    ...
    handleInput(eventType, eventData) {
        ...
        for (const button of this.buttonElements) {
            ...
            const inButton = eventData.x >= buttonBounds.x && 
...
class NavigationMenu extends BaseObject {
    constructor(options = {}) {
        ...
        this.animationDuration = options.animationDuration || UI_CONSTANTS.ANIMATION.DURATION.FAST;
    ...
    bindEvents() {
        try {
            ...
            // Animation events
            this.on('animationStart', () => { this.isAnimating = true; });
            this.on('animationEnd', () => { this.isAnimating = false; });
    ...
    handleInput(eventType, eventData) {
        try {
            ...
            for (let i = 0; i < this.menuItems.length; i++) {
                ...
                if (x >= this.x + itemX && x <= this.x + itemX + itemWidth &&
                    y >= this.y + itemY && y <= this.y + itemY + itemHeight) {
                    hoveredIndex = i;
                    break;
                }
            }
            ...
            switch (eventType) {
                case 'mousemove':
                    if (hoveredIndex !== this.hoveredIndex) {
                        ...
                        this.emit('hoverChanged', { hoveredIndex, hoveredItem: this.menuItems[hoveredIndex] });
                    }
                    break;
                ...
                case 'touchend':
                    if (hoveredIndex >= 0) {
                        this.selectItem(hoveredIndex, eventData.ctrlKey || eventData.metaKey);
                        return true;
                    }
                    break;
            }
    ...
    renderMenuItem(renderer, item, index, itemWidth, itemHeight) {
        ...
        // Text
        const textColor = item.isSelected ? UI_CONSTANTS.COLORS.WHITE : 
                         item.isDisabled ? UI_CONSTANTS.COLORS.GRAY_400 : 
    ...
    renderSubmenuIndicator(renderer, x, y, isSelected) {
        const size = 6;
        const color = isSelected ? UI_CONSTANTS.COLORS.WHITE : UI_CONSTANTS.COLORS.GRAY_600;
    ...
    renderFocusRing(renderer, itemWidth, itemHeight) {
        ...
        renderer.lineWidth = FOCUS_RING_WIDTH;
        renderer.setLineDash([2, 2]);
        renderer.strokeRect(x - FOCUS_RING_WIDTH, y - FOCUS_RING_WIDTH, 
    ...
    getAccessibilityDescription() {
        ...
        return `Navigation menu with ${totalCount} items, ${enabledCount} enabled, ${selectedCount} selected. ` +
...
class Chart extends BaseObject {
    constructor(options = {}) {
        ...
        this.margin = options.margin || { top: 50, right: 40, bottom: 50, left: 70 };
    ...
    processSeriesData() {
        ...
        this.avgValue = allValues.reduce((sum, val) => sum + val, 0) / allValues.length;
    ...
    calculateScales() {
        try {
            switch (this.type) {
                ...
                case 'area':
                case 'bar':
                    this.xScale = this.chartWidth / (this.processedData[0].values.length - 1);
    ...
    findLineDataPoint(x, y, tolerance) {
        for (const series of this.processedData) {
            for (let i = 0; i < series.values.length; i++) {
                ...
                const pointY = this.chartHeight - (series.values[i] - this.minValue) * this.yScale;
    ...
    renderSelf(renderer) {
        ...
        try {
            ...
            renderer.save();
            renderer.beginPath();
            renderer.rect(this.margin.left, this.margin.top, this.chartWidth, this.chartHeight);
            renderer.clip();
    ...
    renderGrid(renderer) {
        ...
        if (this.type !== 'pie') {
            const verticalLines = this.type === 'bar' ? 
    ...
    renderAxes(renderer) {
        ...
        for (let i = 0; i <= ySteps; i++) {
            const value = this.minValue + (i / ySteps) * (this.maxValue - this.minValue);
            const y = this.margin.top + this.chartHeight - (i / ySteps) * this.chartHeight;
        ...
        if (this.labels.length > 0) {
            ...
            this.labels.forEach((label, index) => {
                const x = this.margin.left + (index / (this.labels.length - 1)) * this.chartWidth;
    ...
    renderLineChart(renderer) {
        this.processedData.forEach((series, _seriesIndex) => {
            ...
            renderer.globalAlpha = series.opacity * (this.animated ? this.animationProgress : 1);
    ...
    renderBarChart(renderer) {
        ...
        this.processedData.forEach((series, seriesIndex) => {
            ...
            series.values.forEach((value, index) => {
                const x = this.margin.left + index * barWidth + seriesIndex * seriesBarWidth;
    ...
    renderPieChart(renderer) {
        ...
        this.processedData.forEach((slice, _index) => {
            ...
            renderer.arc(centerX, centerY, animationRadius, slice.startAngle, slice.endAngle);
            renderer.closePath();
            renderer.fill();
    ...
    renderScatterPlot(renderer) {
        this.processedData.forEach((point, _index) => {
            ...
            const y = this.margin.top + this.chartHeight - (point.y - this.minY) * this.yScale;
...
class FormField extends BaseObject {
            ...
            if (this.required) {
                this.addValidationRule('required', (value) => {
                    if (this.type === 'checkbox') {
                        return { isValid: value === true, message: 'This field is required' };
                    }
    ...
    handleSelectableKeyDown(event) {
        ...
        const currentIndex = this.processedOptions.findIndex(opt => opt.value === this.value);
        ...
        if (newIndex !== currentIndex && !this.processedOptions[newIndex].disabled) {
    ...
    renderTextInput(renderer) {
        ...
        if (displayValue) {
            ...
            const textToRender = this.type === 'password' && this.value ? 
                '•'.repeat(this.value.length) : displayValue;
            
            // Truncate text if too long
            const padding = UI_CONSTANTS.SPACING.SM;
            const maxWidth = scaledWidth - (padding * 2);
            const truncatedText = UIUtils.truncateText(textToRender, maxWidth, renderer.font, renderer);
            
            renderer.fillText(truncatedText, offsetX + padding, fieldY + offsetY + scaledHeight / 2);
        }
    ...
    renderCheckbox(renderer) {
        ...
        if (this.label) {
            ...
            renderer.textAlign = 'left';
            renderer.textBaseline = 'middle';
            renderer.fillText(this.label, checkboxSize + UI_CONSTANTS.SPACING.SM, checkboxY + checkboxSize / 2);
        }
    }
    ...
    renderRadioGroup(renderer) {
        ...
        this.processedOptions.forEach((option, index) => {
            ...
            renderer.fillStyle = option.disabled ? 
                ...
                UI_CONSTANTS.COLORS.WHITE;
            renderer.beginPath();
            renderer.arc(radioSize / 2, radioY + radioSize / 2, radioSize / 2, 0, 2 * Math.PI);
            renderer.fill();
            ...
            renderer.textAlign = 'left';
            renderer.textBaseline = 'middle';
            renderer.fillText(option.label, radioSize + UI_CONSTANTS.SPACING.SM, radioY + radioSize / 2);
        });
    }
    ...
    renderCharacterCount(renderer) {
        ...
        const isOverLimit = this.characterCount.max && 
            this.characterCount.current > this.characterCount.max;
    ...
    renderCursor(renderer, x, y, height) {
        ...
        const cursorX = x + UI_CONSTANTS.SPACING.SM + (this.value ? 
            renderer.measureText(this.value).width : 0);
    ...
    renderFocusRing(renderer) {
        ...
        renderer.lineWidth = FOCUS_RING_WIDTH;
        renderer.setLineDash([2, 2]);
        renderer.strokeRect(-FOCUS_RING_WIDTH, fieldY - FOCUS_RING_WIDTH, 
                          this.width + FOCUS_RING_WIDTH * 2, 
...
class Tooltip extends BaseObject {
    ...
    bindTargetEvents() {
        ...
        try {
            ...
            this.target.on('touchcancel', (event) => this.handleTargetTouchCancel(event));
        ...
        try {
            ...
            if (this.followCursor && event) {
                ...
                targetBounds = this.target.getBounds ? 
                    ...
                    { x: this.target.x || 0, y: this.target.y || 0, width: this.target.width || 0, height: this.target.height || 0 };
            } else {
                return;
            }
    ...
    adjustPositionToFit(position) {
        ...
        if (x < padding) {
            x = padding;
        } else if (x + this.calculatedDimensions.width > this.viewportBounds.width - padding) {
        ...
        if (y < padding) {
            y = padding;
        } else if (y + this.calculatedDimensions.height > this.viewportBounds.height - padding) {
            y = this.viewportBounds.height - this.calculatedDimensions.height - padding;
        }
    ...
    renderBackground(renderer) {
        ...
        // Shadow
        renderer.fillStyle = 'rgba(0, 0, 0, 0.1)';
        UIUtils.fillRoundedRect(renderer, 2, 2, this.width, this.height, borderRadius);
        
        // Background
        renderer.fillStyle = bgColor;
        UIUtils.fillRoundedRect(renderer, 0, 0, this.width, this.height, borderRadius);
        
        // Border
        if (borderColor) {
            renderer.strokeStyle = borderColor;
            renderer.lineWidth = 1;
            UIUtils.strokeRoundedRect(renderer, 0, 0, this.width, this.height, borderRadius);
        }
    }
     ...
     */
    getAccessibilityDescription() {
        return `Tooltip with content: "${this.content}". Position: ${this.currentPosition}. ` +
