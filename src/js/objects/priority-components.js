...
export class DataTable extends BaseObject {
    ...
    setupTable() {
        ...
        this.visibleRows = Math.floor((this.height - this.headerHeight) / this.rowHeight);
    ...
    handleClick(event) {
        ...
        if (localY <= this.headerHeight) {
            const columnIndex = Math.floor(localX / (this.width / this.columns.length));
    ...
    handleMouseMove(event) {
        const { localY } = event;
        const newHoveredRow = Math.floor((localY - this.headerHeight) / this.rowHeight);
    ...
    renderPagination(renderer) {
        ...
        // Next button
        renderer.fillStyle = this.currentPage < this.totalPages ? '#2196F3' : '#CCCCCC';
    ...
    getSelectedData() {
        const filteredData = this.getFilteredData();
        return Array.from(this.selectedRows).map(index => filteredData[index]).filter(Boolean);
    }
...
export class LoadingSpinner extends BaseObject {
    ...
    renderSelf(renderer) {
        ...
        if (this.message) {
            ...
            renderer.fillText(this.message, centerX, centerY + this.spinnerSize / 2 + 15);
        }
    ...
    renderProgressCircle(renderer, centerX, centerY) {
        ...
        if (this.progress > 0) {
            ...
            renderer.lineWidth = 4;
            renderer.beginPath();
            renderer.arc(centerX, centerY, radius, -Math.PI / 2, -Math.PI / 2 + progressAngle);
            renderer.stroke();
        }
    }
}
