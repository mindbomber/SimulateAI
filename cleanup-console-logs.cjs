...
function main() {
  ...
  for (const file of files) {
    try {
      ...
      if (removedCount > 0) {
        ...
        console.log(`✅ ${path.basename(file)}: Removed ${removedCount}/${originalCount} console.log statements`);
  ...
  for (const file of files) {
    try {
      ...
      if (matches) {
        ...
        console.log(`⚠️  ${path.basename(file)}: ${matches.length} console.log statements remaining`);
      }
    } catch (error) {
      // Ignore
    }
  ...
  if (remainingConsoleLogCount === 0) {
    ...
    console.log(`⚠️  ${remainingConsoleLogCount} console.log statements still remain`);
  }
}
